...
Recommended workflow from the meta-iot2050 repository root:

```sh
./scripts/host/regenerate-npm-shrinkwraps.sh
```

This refreshes both the Node-RED recipes and the EIO WebUI shrinkwrap files in
one pass.

Manual workflow for this recipe:
...
Run these commands inside a  `debian:<version-snapshot>` container so dependency
resolution matches the ISAR build environment defined in
