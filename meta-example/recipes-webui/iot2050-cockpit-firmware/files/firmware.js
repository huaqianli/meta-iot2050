...
function setMatchStatus (element, current, expected) {
  ...
  element.textContent = matched ? 'Matched' : 'Unmatched';
...
function showError (error) {
  ...
  alert.textContent = error && error.problem === 'access-denied'
    ? 'Administrative access is required. Click the lock icon labeled Limited access, switch to Administrative access, authenticate, then click Refresh.'
...
function renderDeviceInfo (info) {
  document.getElementById('device-details').replaceChildren(
    ...
    detail('MLFB:', info.mlfb),
    detail('SN:', info.serial),
  );
...
function applyModuleInspection (inspection) {
  document.getElementById('firmware-a').disabled = !inspection.chip_a_node;
  document.getElementById('firmware-b').disabled = !inspection.chip_b_node;
...
function setWriteControlsDisabled (disabled) {
  document.getElementById('update-system').disabled = disabled || providerAvailability.system === false;
  document.getElementById('update-controller').disabled = disabled || providerAvailability.controller === false;
  document.getElementById('update-module').disabled = disabled || providerAvailability.module === false;
}
...
function setModuleControlsDisabled (disabled) {
  ...
  document.getElementById('firmware-a').disabled = disabled;
  document.getElementById('firmware-b').disabled = disabled;
...
function showUnavailable (statusElement, detailsElement, reason) {
  ...
  detailsElement.replaceChildren(detail('Reason', reason || 'Provider is unavailable'));
}
...
async function pollTask (taskId) {
  ...
  document.getElementById('task-message').textContent = (task.error && task.error.message) || task.phase;
  ...
  safety.textContent = task.state === 'running'
    ...
    : task.state === 'succeeded'
      ...
      : task.provider === 'controller' || task.provider === 'module'
  ...
  if (task.state === 'running') {
    ...
    if (task.state === 'failed' && task.provider === 'system' &&
        task.operation === 'update') {
      try {
...
async function loadCapabilities () {
  ...
  try {
    ...
    const capabilityMap = new Map(capabilities.map(capability => [capability.provider, capability]));
    providerAvailability = Object.fromEntries(
      capabilities.map(capability => [capability.provider, capability.available !== false]),
    );
    const systemCapability = capabilities.find(capability => capability.provider === 'system');
    document.getElementById('system-file-hint').textContent = (systemCapability && systemCapability.default_package)
